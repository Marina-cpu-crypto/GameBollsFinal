...
namespace GameBolls3._0
{
    public class GameProcess
    {
        ...
        //{
        //    if (level == Level1) return 6;
        //    else return 8;
        //}

    private readonly Dictionary<string, Image> ballImages = new Dictionary<string, Image>(); //Вместо отдельных переменных 

        public GameProcess()
        {
            ballImages[Empty] = Bitmap.FromFile("Images/Empty.png");
            ballImages[RedBall] = Bitmap.FromFile("Images/RedBaloon_100.png");
            ballImages[OrangeBall] = Bitmap.FromFile("Images/OrangeBaloon_100.png");
            ballImages[BlueBall] = Bitmap.FromFile("Images/BlueBallon_100.png");
        }
        ...
        public void ChooseBall(string level, DataGridView grid, string[,] gameField, int row, int column)
        {
            ...
            grid.Rows[row].Cells[column].Value = ballImages[Empty];
            grid.Rows[SelectedBallRow].Cells[selectedColumn].Value = ballImages[ballType];
        }
        ...
        public bool PlaceBall(string level, DataGridView grid, string[,] gameField, int row, int column)
        {
            ...
            grid.Rows[row].Cells[column].Value = ballImages[ballType];
            grid.Rows[SelectedBallRow].Cells[selectedColumn].Value = ballImages[Empty];
